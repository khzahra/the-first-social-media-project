...
class UserRegisterViews(View):
    ...
    def get(self, request):
        """
        check the authentication before showing form
        if request.user.is_authenticated:
            return redirect("home:home")
        """
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                return redirect('home:home')
            messages.error(request, "Check your information again.", 'warning')
        return render(request, self.template_name, {'form': form})
...
class UserFollowView(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        relation = Relation.objects.filter(from_user=request.user, to_user=user)
        if relation.exists():
            messages.error(request, 'You already followed this account.', 'danger')
        else:
...
class UserUnfollowView(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        if relation.exists():
            ...
            messages.error(request, "You have not followed this account yet.", 'danger')
        return redirect("account:user_profile", user.id)
